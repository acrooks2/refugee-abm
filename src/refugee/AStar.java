...
//       
        ...
        // A* search
        HashMap<MigrationBuilder.Node, AStarNodeWrapper> foundNodes =
                new HashMap<MigrationBuilder.Node, AStarNodeWrapper>();
        ...
        while(openSet.size() > 0){ // while there are reachable allRoadNodes to investigate
            ...
            for(Edge l: x.node.links){
                ...
                MigrationBuilder.Node n = (MigrationBuilder.Node) l.from();
                if( n == x.node )
                    n = (MigrationBuilder.Node) l.to();
...
//                System.out.println("Time = " + System.currentTimeMillis());
     ...
     */
    public static List<MigrationBuilder.Node> getNodesWithinDistance(MigrationBuilder.Node start, Map endNodes, double max_distance, double speed)
    {
        //        int[] cacheKey = new int[] {start.location.xLoc, start.location.yLoc, goal.location.xLoc, goal.location.yLoc};
//        if (cache.containsKey(cacheKey))
//            return cache.get(cacheKey);
//
        ...
        List<MigrationBuilder.Node> nodesToReturn = new LinkedList<>();
        ListIterator<MigrationBuilder.Node> listIterator = nodesToReturn.listIterator();//pointer to last position in the nodesToReturn
...
//                System.out.println("Time = " + System.currentTimeMillis());
     ...
     * @return
     */
    public static Route getNodeAtDistance(MigrationBuilder.Node start, double distance, double speed)
    {
        //        int[] cacheKey = new int[] {start.location.xLoc, start.location.yLoc, goal.location.xLoc, goal.location.yLoc};
//        if (cache.containsKey(cacheKey))
//            return cache.get(cacheKey);
...
//                System.out.println("Time = " + System.currentTimeMillis());
     ...
     */
    static double heuristic(MigrationBuilder.Node x, MigrationBuilder.Node y) {
